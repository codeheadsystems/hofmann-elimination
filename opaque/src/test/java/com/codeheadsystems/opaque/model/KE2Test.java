...
class KE2Test {
  ...
  void deserializeAssignsFieldsAtCorrectOffsets() {
    ...
    int off = 0;
    fill(wire, off, noe,    (byte) 0x01); off += noe;
    fill(wire, off, nn,     (byte) 0x02); off += nn;
    fill(wire, off, masked, (byte) 0x03); off += masked;
    fill(wire, off, nn,     (byte) 0x04); off += nn;
    fill(wire, off, npk,    (byte) 0x05); off += npk;
