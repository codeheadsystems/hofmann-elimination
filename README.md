...
for common services to reduce their attack surfaces include offline attacks 
from stollen credentials. Usable with standard frameworks like Dropwizard and Spring Boot.
...
This project also provides the OPRF primitive on its own as well. This allows clients
...
data produced it. Useful when that data is sensitive information the client wants to keep private.
...
| Module                | Contents                                                                         |
|-----------------------|----------------------------------------------------------------------------------|
| `hash-to-curve/`      | Core elliptic curve classes to implement RFC 9380 (hash-to-curve) and utilities  |
...
validated against the RFC's own test vectors (Appendix J.7.1 for secp256k1 and 
Appendix A.1.1 for P-256).
...
| Cipher suite           | Multiple Weierstrass curves incl. P-256/SHA-256, with Ristretto planned | Ristretto255       | Ristretto25519     | Multiple incl. P-256  |
