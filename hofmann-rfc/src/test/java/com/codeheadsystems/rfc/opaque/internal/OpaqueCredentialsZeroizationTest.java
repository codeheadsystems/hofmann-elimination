...
class OpaqueCredentialsZeroizationTest {
  ...
  /**
   * The identity KSF returns its input array rather than a copy, so {@code oprfOutput} and
   * {@code stretchedOutput} are the same object and the clearing code zeroes it twice. Harmless,
   * but only because the concatenation that feeds the extract step is taken first — this pins
   * that ordering, since reversing it would produce a randomized password derived from zeros.
   */
  @Test
  void deriveRandomizedPwd_isUnaffectedByTheIdentityKsfAliasingItsInput() {
    ...
    byte[] first = OpaqueCredentials.deriveRandomizedPwd(PASSWORD, blind, evaluated, identity);
    byte[] second = OpaqueCredentials.deriveRandomizedPwd(PASSWORD, blind, evaluated, identity);

    assertThat(first).isEqualTo(second).isNotEqualTo(new byte[first.length]);
  }
}
