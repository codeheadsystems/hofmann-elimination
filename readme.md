...
This project provides a protocol to create consistent, reusable identifiers across multiple
clients without sharing the key material that generated them. The result is an identifier that
services can use without learning what data produced it. Useful when that data is sensitive
information the client wants to keep private.

The implementation covers RFC 9497 OPRF(P-256, SHA-256) mode 0 (using RFC 9380 hash-to-curve
techniques), with an example client/server implementation in Java. These primitives are then
used to implement the OPAQUE Augmented Password-Authenticated Key Exchange (aPAKE) protocol
...
| Module                | Contents                                                                     |
|-----------------------|------------------------------------------------------------------------------|
| `elliptic_curve/`     | Core elliptic curve classes including RFC 9380 (hash-to-curve) and utilities |
...
- `Stretch`: The Key Stretching Function (KSF); currently Identity (no-op). May be replaced with scrypt or Argon2.
